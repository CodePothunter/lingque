...
REFLECTION_WITH_CURIOSITY_PROMPT = (
    ...
    "- 猜测用户心理（如\"用户为什么这样问\"）\n"
    "- 上下文疑问（如\"用户之前说了什么\"）\n"
    "- 纯粹客套（如为了完成JSON格式而编问题）\n\n"
    "【正例参考】真正的好奇心：\n"
    "- \"心跳退避机制的具体算法是什么\"（指向可学习的技术知识）\n"
    "- \"如何用Python实现指数退避\"（指向可执行的技能）\n"
    "- \"wttr.in API的返回格式是怎样的\"（指向可查询的API文档）\n\n"
    "【反例警示】不是真正的好奇心：\n"
    "- \"用户之前究竟提到了什么话题\"（上下文追问，不是知识盲区）\n"
    "- \"为什么用户对公网IP感到困惑\"（猜测用户心理，无法学习）\n"
    "- \"用户知道内网IP却不知道公网IP\"（观察陈述，没有问题）\n\n"
    "真正的好奇心应该指向你可以学习的具体知识或技能。\n\n"
    "你的回复：\n{reply}"
)
