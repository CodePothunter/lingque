...
| 维度 | OpenAI Assistants | LangChain Agent | Manus | OpenClaw | **灵雀** |
|------|------------------|-----------------|-------|----------|---------|
| 世界模型 | 无 | 无 | 无 | 无（会话压缩） | **五个核心文件** |
| 奖励来源 | 无 | 无 | 外部（任务完成） | 无 | **内部（三维 LLM 评估）** |
| 价值函数 | 无 | 无 | 无 | 无 | **Bellman + LLM 估值** |
| 策略更新 | 无 | 无 | 无 | 无 | **PPO clip + 自然语言约束** |
| 任务选择 | 人类指定 | 人类指定 | 人类指定 | 人类指定 | **Thompson Sampling** |
| 可解释性 | 低 | 中 | 低 | 中 | **完全透明（自然语言）** |
| 持续进化 | 无 | 无 | 无 | 无（工具扩展） | **有（进化日志 + checkpoint）** |
| 人格稳定性 | N/A | N/A | N/A | 弱（compaction 导致漂移） | **PPO 策略守卫** |
