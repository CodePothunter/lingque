...
奶油（灵雀实例之一）从 2026 年 1 月上线，30 天内从新手成长为专家：

| 阶段 | 策略特征 | 行为表现 |
|------|----------|----------|
| **v1（第1天）** | 均匀策略 | 什么都想试，工具使用随机 |
| **v23（第10天）** | 开始有偏好 | 学会优先查记忆再回答 |
| **v45（第20天）** | 反思型策略 | 先分析问题类型，再选工具 |
| **v68（第30天）** | 专家型策略 | 先反思再回答，自主判断何时翻源码、何时查记忆 |

**关键进化点**：
- 第 5 天：学会"不确定时先查 MEMORY.md"
- 第 12 天：学会"复杂代码问题先 run_bash 看结构"
- 第 25 天：学会"主人不在线时主动做心跳任务，不浪费唤醒"
- 第 30 天：策略收敛稳定，每次心跳都有明确目标

> **不是人工调的 prompt，是自己从经验中学到的策略。** 这就是强化学习的力量。
