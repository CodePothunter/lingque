...
- **router/** — Core routing package (split by responsibility into 8 modules). `core.py` defines `MessageRouter` and event dispatch; `defs.py` holds tool definitions; `private.py` handles private chat + reflection; `group.py` implements three-layer group chat intervention; `tool_loop.py` runs the agentic tool-use loop; `tool_exec.py` dispatches tool execution + multimodal; `web_tools.py` covers web search/fetch via MCP; `runtime_tools.py` covers Python execution, file I/O, and self-stats. Depends on `PlatformAdapter` (not Feishu SDK).
